...
//定数
const wchar_t* pClassName = L"DxBase2016Class";
const wchar_t* pWndTitle = L"DxBase2016Sample";
...
int MainLoop(HINSTANCE hInstance, HWND hWnd, bool isFullScreen, int iClientWidth, int iClientHeight){
	...
	try{
		...
		if (FAILED(CoInitializeEx(nullptr, COINIT_MULTITHREADED))){
			...
			throw BaseException(
				L"Com初期化に失敗しました。",
				L"FAILED(CoInitializeEx))",
				L"WinMain()"
				);
		}
		////アプリケーションクラスの構築
		App::GetApp(hInstance, hWnd, isFullScreen, iClientWidth, iClientHeight);
		//シーンの追加登録
