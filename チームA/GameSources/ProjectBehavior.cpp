...
namespace basecross {
	...
	///	プレイヤーの行動クラス
	//--------------------------------------------------------------------------------------
	Vector3 PlayerBehavior::GetMoveVector_Second() const {
		Vector3 Angle(0, 0, 0);
		//コントローラの取得
		auto CntlVec = App::GetApp()->GetInputDevice().GetControlerVec();
		if (CntlVec[0].bConnected) {
			if (CntlVec[0].fThumbRX != 0 || CntlVec[0].fThumbRY != 0) {
				...
				float MoveZ = CntlVec[0].fThumbRY;
				Vector2 MoveVec(MoveX, MoveZ);
				float MoveSize = MoveVec.Length();
				//コントローラの向きから角度を計算
				float CntlAngle = atan2(-MoveX, MoveZ);
	...
	void PlayerBehavior::MovePlayer_Second() {
		float ElapsedTime = App::GetApp()->GetElapsedTime();
		Vector3 Angle = GetMoveVector_Second();
		//Rigidbodyを取り出す
		auto PtrRedit = GetGameObject()->GetComponent<Rigidbody>();
		auto Velo = PtrRedit->GetVelocity();
		if (Angle.Length() <= 0.0f && Velo.y == 0.0f) {
